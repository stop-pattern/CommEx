...
## CommEx 現行実装メモ（2026-04-03）

- COM 通信は `ComTransportModel` で設定を保持し、`Ports` 配列の各要素（1ポート）ごとに独立ワーカーを起動する。
- 各ワーカーは専用キュー（容量制限あり）を持ち、メインスレッドからのスナップショット投入をノンブロッキングで受ける。
- ワーカーは `TaskCreationOptions.LongRunning` で別スレッド起動し、シリアル I/O をメインスレッドから分離する。
- 停止時はキュー完了通知とキャンセルで安全に終了する。

...
リモート管理にはGitHubを使用します。[CI/CD仕様](specs/000-product/ci-cd.md)では、push時の
コードチェック・ビルド・DLL成果物のアップロード（保管1日）と、tag push時の同じ処理に続く
ドラフトRelease作成・生成物添付の2つのワークフローを定義しています。
これは実装予定の仕様です。ドラフト作成だけで製品の正式公開に必要な検証を完了した扱いにはしません。
...
- 意味のある最小単位が検証できたらその都度コミットします。1プロンプト1コミットではありません。
  タスクごとにコミットを分け、別タスクや既存の未コミット変更を混ぜません。同じタスクに必要な
  実装・テスト・文書群はまとめ、1タスクを複数の検証済みコミットに分けることもできます。
  作業前に既存タスクと担当・変更範囲を調整します。詳細は[開発・再開ガイド](docs/development-workflow.md)を参照してください。
